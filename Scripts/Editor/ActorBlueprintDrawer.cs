    ...
    public class ActorBlueprintDrawer : Editor
    {
        ...
        private void OnEnable()
        {
            _componentTypesCache = AppDomain.CurrentDomain.GetAssemblies()
                ...
                .Where(t => t.IsSubclassOf(typeof(BaseComponentProvider)) && !t.IsAbstract).ToList();

            _behaviourTypesCache = AppDomain.CurrentDomain.GetAssemblies()
                .SelectMany(t => t.GetTypes())
                .Where(t => t.IsSubclassOf(typeof(BaseBehaviourProvider)) && !t.IsAbstract).ToList();
            ...
            _componentsProperty = serializedObject.FindProperty("_components");
            _behavioursProperty = serializedObject.FindProperty("_behaviours");
            _componentsList = new ReorderableList(serializedObject, _componentsProperty, true, true, true, true);
            _behavioursList = new ReorderableList(serializedObject, _behavioursProperty, true, true, true, true);

            _componentsList.drawHeaderCallback = (rect) => { EditorGUI.LabelField(rect, "_components"); };
            _componentsList.drawElementCallback = DrawComponentElement;
            _componentsList.elementHeightCallback = GetComponentElementHeight;
            _componentsList.onAddCallback = AddComponent;
            _componentsList.onRemoveCallback = RemoveElement;
        ...
        private void DrawComponentElement(Rect rect, int index, bool isActive, bool isFocused, SerializedProperty element)
        {
            ...
            var provider = element.objectReferenceValue as BaseComponentProvider;
            var label = "";
            if (provider)
        ...
        private void DrawBehaviourElement(Rect rect, int index, bool isActive, bool isFocused, SerializedProperty element)
        {
            ...
            var provider = element.objectReferenceValue as BaseBehaviourProvider;
            var label = "";
            if (provider)
        ...
        private void AddComponent(ReorderableList l)
        {
            ...
            _searchComponentsProvider.EntrySelected = (o) =>
            {
                ...
                {
                    var instance = (BaseComponentProvider)ScriptableObject.CreateInstance(type);
        ...
        private void AddBehaviour(ReorderableList l)
        {
            ...
            _searchBehavioursProvider.EntrySelected = (o) =>
            {
                ...
                if (blueprint != null && type != null)
                {
                    var instance = (BaseBehaviourProvider)ScriptableObject.CreateInstance(type);
                    instance.name = type.Name;
                    list.InsertArrayElementAtIndex(index);
                    var el = list.GetArrayElementAtIndex(index);
                    AssetDatabase.AddObjectToAsset(instance, blueprint);
